...
def simple_registry():
    ...
    reg.register_as_structure(
        SerializableObject, "SerializableObject", scope=PortScope.LOCAL
    )
    reg.register_as_structure(
...
def test_define_nested_basic_function(simple_registry):
    ...
    assert (
        functional_definition.args[1].children[0].kind == PortKind.INT
    ), "Child of List is not of type IntArgPort"
    assert (
        functional_definition.args[0].children[0].kind == PortKind.STRING
...
def test_define_nested_structure_function(simple_registry):
    ...
    assert (
        functional_definition.args[0].children[0].kind == PortKind.STRUCTURE
    ), "Child of List is not of type IntArgPort"
    assert (
        functional_definition.args[0].children[0].identifier == "SerializableObject"
    ), "Child of List is not of type IntArgPort"
    assert (
        functional_definition.args[0].children[0].kind == PortKind.STRUCTURE
    ), "Child of Dict is not of type StringArgPort"
    ...
    assert (
        functional_definition.returns[1].children[0].identifier == "SecondSerializableObject"
    )
